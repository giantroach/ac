...
## cards-boss-action.jsonc
裏面はブランク。表面には id のみを大きく表示するシンプルなカード。
セットアップ時にシャッフルされ、すべて裏向きで Shared Play Area のグリッドに並べて配置される。
- id (string)
...
5. 画面の左端上部にボスの情報を表示する。現在のHPが参照できる。また、ここに小さな+と-のボタンを配置し、HPの値を変更できるようにする。さらに「Reset Actions」ボタンを配置し、押下するとboss-actionデッキを再シャッフルして Shared Play Area に裏向きで再配置する。
...
6. cards-boss-action.jsonc に定義されたカードデッキをシャッフルし、すべてを裏向きで Shared Play Area のグリッドに左から順に配置する。
